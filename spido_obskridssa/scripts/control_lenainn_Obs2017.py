...
def NLObserverBFBr(y,theta_tilde,DeltaF,DeltaR,vitesse,C,Epsi,DCapR,theta_tilde_dot):
    ...
    if vitesse<=0.5:
        ...
        C=pres*np.array([[-cos(DeltaR+theta_tilde)*alphaob,sin(DeltaR+theta_tilde)],[-sin(DeltaR+theta_tilde)*alphaob,-cos(DeltaR+theta_tilde)]])
        CM=C.dot(B)  
...
def LinearKinmaticModel(y_obs,theta_tilde_obs,Xm,v,DeltaF,C,betaF,betaR):
    ...
    return  y_obs,theta_tilde_obs,Xm,fFB00
...
def LenainObsvBFBr(y,theta_tilde,y_obs,theta_tilde_obs,DeltaF,DeltaR,v,C,theta_tilde_dot,y_dot,fFB00,fF):
    global Tsamp,LR,LF,L,betaRold,betaFold
    if C==0:
            C=0.00000001;
    B11=0;
...
#    We=np.linalg.pinv(B).dot(G.dot(epsilon)-f+X_dot)
    We=np.linalg.pinv(B).dot(G.dot(epsilon)-fFB00+fF)
    betaF=We[0,0]
    betaR=We[1,0]
...
def talker():
    ...
    ref=np.loadtxt('/home/rcse/catkin_ws/src/spido_obskridssa/scripts/matS.txt')
    ref[:,1]=ref[:,1]-ref[0,1]
    ref[:,2]=ref[:,2]-ref[0,2]
    dpsi=Psi-ref[0,3]
    ref[:,3]=ref[:,3]+dpsi
    ...
    while ((rospy.get_time()-t0<=simul_time)):
        ...
        ### For Roland 2017
        fF=KinmaticModel(betaF,betaR,y,theta_tilde,vitesse,DeltaF,c)
        y_obs,theta_tilde_obs,Xm,fFB00=LinearKinmaticModel(y_obs,theta_tilde_obs,Xm,vitesse,DeltaF,c,betaF,betaR)
        betaF,betaR=LenainObsvBFBr(y,theta_tilde,y_obs,theta_tilde_obs,DeltaF,DeltaR,vitesse,c,theta_tilde_dot,y_dot,fFB00,fF)
        ...
        betaF=0 
        betaR=0
        #betaF,betaR,Epsi=LinearObsvBFBr(y,theta_tilde,DeltaF,DeltaR,vitesse,c,Epsi,DCapR,theta_tilde_dot)   #  Linear Observer
#        betaF,betaR,Epsi=NLObserverBFBr(y,theta_tilde,DeltaF,DeltaR,vitesse,c,Epsi,DCapR,theta_tilde_dot)   # NL  Linear Observer
        
#        y_obs,theta_tilde_obs,Xm=KinmaticModel(betaF,betaR,y,theta_tilde,Xm,vitesse,DeltaF,c)
#        betaF,betaR=LenainObsvBFBr(y,theta_tilde,y_obs,theta_tilde_obs,DeltaF,DeltaR,vitesse,c,theta_tilde_dot,y_dot)
#        
        
        ###   Adaptative controller 
        u=deltaFunction( y, theta_tilde, c,betaF,betaR)
        DeltaF=u
        ...
        posture = np.array([X,Y,Psi,y,theta_tilde,Psip,u,xref,yref,Psiref,betaF], dtype=np.float32)
