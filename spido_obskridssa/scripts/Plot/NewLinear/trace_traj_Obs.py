...
NLobs=np.loadtxt('/home/summit/Spidoo_ws/src/spido_obskridssa/scripts/Plot/NLObser_6.txt') #NLobser_5
LinObs=np.loadtxt('/home/summit/Spidoo_ws/src/spido_obskridssa/scripts/Plot/LinObser_6.txt')
WithoutObs=np.loadtxt('/home/summit/Spidoo_ws/src/spido_obskridssa/scripts/Plot/Without_contr_6.txt')
NewLin=np.loadtxt('/home/summit/Spidoo_ws/src/spido_obskridssa/scripts/Plot/NewLinear/New_Line_V6.txt')
...
XNewLIn= butter_lowpass_filter(NewLin[:,1], 0.04, 10, 8)
YNewLIn= butter_lowpass_filter(NewLin[:,2], 0.04, 10, 8)
...
plt.plot(XNewLIn ,YNewLIn , 'c--', linewidth=3.20)
...
#
plt.ylim((-30, 120))   # set the xlim to xmin, xmax

plt.grid(True)
plt.legend(["Reference path","Path without observer at $V_x=4 m.s^{-1}  $","Path for linear observer at $V_x=4 m.s^{-1}$","Path for non-linear observer at $V_x=4 m.s^{-1}  $","Path for New non-linear observer at $V_x=4 m.s^{-1}  $"],loc='upper left',fontsize=30)
plt.show()
...
i=0
while (i<len(NewLin[:,1])-1):
        s66[i+1]=s66[i]+hypot(NewLin[i+1,1]-NewLin[i,1],NewLin[i+1,2]-NewLin[i,2])
	i=i+1
...
plt.grid(True)
plt.legend(["y without observer at $V_x=4 m.s^{-1}$ ","y linear observer at $V_x=4 m.s^{-1}$","y non-linear  observer at $V_x=4 m.s^{-1}$","y New non-linear observer at $V_x=4 m.s^{-1}  $"],loc='upper left',fontsize=30)
#plt.title('L')
...
SSAFRLnEW= butter_lowpass_filter(NewLin[:,11], 0.06, 200, 6)
SSARRLnEW= butter_lowpass_filter(NewLin[:,12], 0.06, 200, 6)

plt.plot(s8[:]-3.7*np.ones((len(s8))),SSAFRL*kk*2, 'r', linewidth=3.2)   
plt.plot(s4[:],SSAF*kk, 'b', linewidth=3.2)
plt.plot(s66[:],SSAFRLnEW*kk, 'k', linewidth=3.2)
plt.plot(s8[:]-2*np.ones((len(s8))),SSARRL*kk*2, 'r--', linewidth=3.2)
plt.plot(s4[:],SSAR*kk, 'b--', linewidth=3.2)
plt.plot(s66[:],SSARRLnEW*kk, 'k--', linewidth=3.2)


plt.xlabel('Curvilinear abscissa  [m]', fontsize=35)
plt.ylabel('Slip angles [deg]', fontsize=35)
plt.grid(True)
plt.legend(["$\\beta_f$ at $V=4 m.s^{-1} $ Linear observer","$\\beta_f$ at $V=4 m.s^{-1} $ non-linear observer","$\\beta_f$ at $V=4 m.s^{-1} $ New non-linear observer","$\\beta_r$ at $V=4 m.s^{-1} $ linear observer","$\\beta_r$ at $V=4 m.s^{-1} $ non-linear  observer","$\\beta_r$ at $V=4 m.s^{-1} $ New non-linear observer"],loc='upper left',fontsize=30)
plt.yticks( color='k', size=42)
plt.xticks( color='k', size=42)
...
yf66= butter_lowpass_filter(NewLin[:,7], 0.04, 200, 6)
...
plt.plot(s66[:],180/pi*yf66, 'g', linewidth=3)


plt.xlabel('Curvilinear abscissa  [m]', fontsize=35)
plt.ylabel('Steering angles [deg]', fontsize=35)
plt.grid(True)
plt.legend(["$\\delta_f$ at $V=4 m.s^{-1} $ without observer","$\\delta_f$ at $V=4 m.s^{-1} $ linear observer","$\\delta_f$ at $V=4 m.s^{-1} $ non-linear observer","$\\delta_f$ at $V=4 m.s^{-1} $ New non-linear observer"],loc='upper left',fontsize=30)
